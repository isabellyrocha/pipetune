...
class MyTuner(kerastuner.tuners.Hyperband):
    def run_trial(self, trial, *args, **kwargs):
        kwargs['batch_size'] = trial.hyperparameters.Choice('batch_size', values=[32, 64, 256, 1024])
...
def build_model(hp):
    ...
    cnn_model.add(Conv1D(64, kernel_size=(8,),strides=2,activation='linear',input_shape=(40, 72),padding='same'))
    ...
    for i in range(0, 3-1):                                                 
        cnn_model.add(Conv1D(64, kernel_size=(8,),strides=2,activation='linear',padding='same'))
        cnn_model.add(BatchNormalization())                                                     
        cnn_model.add(LeakyReLU(alpha=0.1))                                                     
        #cnn_model.add(MaxPooling1D((2,),padding='same'))                                            
        cnn_model.add(Dropout(0.5))                                                  
    for i in range(0, 3):                                                   
        cnn_model.add(Conv1D(64, (6,), strides=2,activation='linear',padding='same'))
        cnn_model.add(BatchNormalization())                                                     
        cnn_model.add(LeakyReLU(alpha=0.1))                                                     
        #cnn_model.add(MaxPooling1D(pool_size=(2,),padding='same'))                                  
        cnn_model.add(Dropout(0.5))                                                  
    for i in range(0, 3):                                                   
        cnn_model.add(Conv1D(64, (4,), activation='linear',padding='same'))      
        cnn_model.add(BatchNormalization())                                                     
        cnn_model.add(LeakyReLU(alpha=0.1))                                                     
        #cnn_model.add(MaxPooling1D(pool_size=(2,),padding='same'))                                  
        cnn_model.add(Dropout(0.5))                                                  
    for i in range(0, 3):                                                   
        cnn_model.add(Conv1D(64, (2,), strides=2,activation='linear',padding='same'))
        cnn_model.add(BatchNormalization())                                                     
        cnn_model.add(LeakyReLU(alpha=0.1))                                                     
        #cnn_model.add(MaxPooling1D(pool_size=(2,),padding='same'))                                  
        cnn_model.add(Dropout(0.5))
...
tuner.search(train[0], train[1], validation_data=(test[0], test[1]))
